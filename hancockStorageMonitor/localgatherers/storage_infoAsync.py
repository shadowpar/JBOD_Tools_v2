from .scsiOperations import scsiAddr2Model, sortSCSIDisks, getMountMap, scsiAddr2SG, scsiAddr2BlockDevName,  scsiAddr2SerialNumberAsync, blockDevName2SerialNumberAsync
from .scsiOperations import multiExecute, getMDparent, generateSAS2SerialMap, generateSAS2SerialMapAsync, multiExecuteAsync, scsiAddr2SASaddr
...
class storage_infoAsync(object):
    def __init__(self):
        super(storage_infoAsync, self).__init__()
        ...
        self.scsi2name = self.getSCSINames()
        self.name2serials = self.getSCSISerials()
        self.scsi2sas = self.getSCSI2SASAddrs()
        self.SAS2SerialByChassis = self.createSAS2SerialsMap()
        ...
        for serial in self.serials:
            for encl in self.disksByChassisEnclosure[serial]:
                self.sesController2modelMap[encl] = scsiAddr2Model(encl)
        for serial in self.disksByChassisEnclosure:#loop over physical enclosures identified by serial number.
            chassis_encls = list(self.disksByChassisEnclosure[serial].keys())#  create a list of SCSI addresses of SES controllers.
            if self.sesController2modelMap[chassis_encls[0]] in NEWJBODMODELS:
                self.jbod_objects[serial] =  storage_jbod_new(uniqueID=serial, sesControllers=chassis_encls)
            else:
                self.jbod_objects[serial] = storage_jbod_old(uniqueID=serial, sesControllers=chassis_encls,disksByEnclosure=self.disksByChassisEnclosure[serial],
                                                             disksas2serial=self.SAS2SerialByChassis[serial])
            self.name2type[serial] = 'jbods'
        self.createharddrives()

    def createSAS2SerialsMap(self):
        data = {}
        for jserial in self.disksByChassisEnclosure:
            data[jserial] = {}
            for ses in self.disksByChassisEnclosure[jserial]:
                for scsiaddr in self.disksByChassisEnclosure[jserial][ses]:
                    sas = self.scsi2sas[scsiaddr.name]
                    name = self.scsi2name[scsiaddr.name]
                    serial = self.name2serials[name]
                    data[jserial][sas] = serial
        return data

    def getSCSI2SASAddrs(self):
        kwargs = [{'scsiaddr':scsiaddr} for scsiaddr in self.scsi2name]
        results = multiExecute(scsiAddr2SASaddr,kwargs=kwargs).values()
        data = {tup[0]:tup[1]  for tup in results}
        return data


    def getSCSINames(self):
        data = {}
        for jserial in self.disksByChassisEnclosure:
            for ses in self.disksByChassisEnclosure[jserial]:
                kwargs = [{'scsiaddr':disk.name} for disk in self.disksByChassisEnclosure[jserial][ses]]
                results = list(multiExecute(functionName=scsiAddr2BlockDevName,kwargs=kwargs).values())
                for tup in results:
                    if tup[0] is not None:
                        data[tup[0]] = tup[1]
        return data

    def getSCSISerials(self):
        data = {}
        kwargs = [{'blockName':name} for name in self.scsi2name.values() if name is not None]
        results = list(multiExecuteAsync(blockDevName2SerialNumberAsync,kwargs=kwargs).values())
        for tup in results:
            data[tup[0]] = tup[1]
        return data
        ...
        for chassisSerial in self.disksByChassisEnclosure:
            ...
            for ses in self.disksByChassisEnclosure[chassisSerial]:
                ...
                for disk in self.disksByChassisEnclosure[chassisSerial][ses]:
                    ...
                    argset.append({'sdname':sdname,'scsiaddr':scsiAddr,'serialnumber':None})
                    self.name2type[sdname] = 'logicaldisks'
                results = []
                for i in range(len(argset)):
                    results.append(storage_logical_disk(**argset[i]))
                # results = multiExecute(functionName=storage_logical_disk,kwargs=argset)
                # values = list(results.values())
                self.logicalDisksByChassisEnclosure[chassisSerial][sesSG] = results
